"""Run manifest-based coffee price crawlers for the official dataset."""
...
def ensure_canonical_output_dir(path: Path) -> None:
    normalized = path.as_posix().rstrip("/")
    if normalized in {"", ".", "data"}:
        raise ValueError(f"Refusing to write crawl output to broad path {path}")
...
async def run_all_sites(args: argparse.Namespace) -> None:
    ...
    ensure_canonical_output_dir(output_dir)
    ensure_canonical_output_dir(raw_dir)
    ensure_canonical_output_dir(error_dir)
