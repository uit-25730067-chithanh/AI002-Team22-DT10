...
from crawler import build_area_datasets
from crawler import data_audit
...
def test_build_canonical_dataset_writes_canonical_filename(tmp_path: Path) -> None:
    raw_dir = tmp_path / "raw" / "canonical"
    output_root = tmp_path / "processed"
    write_raw_inputs(raw_dir)

    result = build_area_datasets.build_canonical_dataset(
        raw_dir,
        output_root,
        "monthly",
    )
    ...
    assert list((output_root / "monthly").glob("coffee_environment_all_areas_monthly_*.csv")) == [output]
    assert len(result) == 1
...
def test_audit_helper_summarizes_old_and_new_by_province() -> None:
    ...
    summary = data_audit.compare_observed_data_completeness(
        old,
        new,
        ["province"],
    )
