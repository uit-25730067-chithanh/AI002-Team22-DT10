...
from crawler import run_independent_price_crawlers as runner
...
def test_runner_refuses_legacy_raw_output_dir() -> None:
    with pytest.raises(ValueError, match="Refusing to write independent crawl"):
        runner.ensure_independent_output_dir(Path("data/raw"))


def test_runner_accepts_independent_output_dir() -> None:
    runner.ensure_independent_output_dir(contract.RAW_DIR)


def test_merge_source_into_raw_uses_independent_combined_filename(tmp_path: Path) -> None:
